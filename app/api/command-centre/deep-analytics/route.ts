...
export async function GET(req: NextRequest) {
  ...
  const period = Math.min(365, Math.max(1, parseInt(req.nextUrl.searchParams.get("period") ?? "30", 10) || 30));
  const now = new Date();
  const from = new Date(now.getTime() - period * 86_400_000);
  const priorFrom = new Date(from.getTime() - period * 86_400_000);
    ...
    showOpen2026
      ? (async () => {
          const unclaimedIds = await prisma.business.findMany({
            where: {
              claimed: false,
              category: { slug: { in: ["hotels", "accommodation", "restaurants", "pubs", "golf"] } },
            },
            select: { id: true },
          });
  ...
  const categoryMatrix = categories.map((cat) => {
    ...
    const topBiz = bizInCat
      .map((b) => ({
        b,
        total:
          (clickMapCurrent.get(b.id)?.view ?? 0) +
          (clickMapCurrent.get(b.id)?.website ?? 0) +
          (clickMapCurrent.get(b.id)?.phone ?? 0) +
          (clickMapCurrent.get(b.id)?.directions ?? 0),
      }))
