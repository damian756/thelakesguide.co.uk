...
export function getUpgradeNudgeHtml(params: {
  ...
  const shareLine =
    categorySharePercent != null && categorySharePercent > 0
      ? `That's ${categorySharePercent}% of all ${categoryName} searches in the Lake District.`
      : null;
...
<head>
  ...
  <title>${businessName} on The Lakes Guide</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #f5f5f5;">
  <div style="max-width: 560px; margin: 0 auto; padding: 24px;">
    <div style="background: white; border-radius: 12px; padding: 24px; border: 1px solid #eee;">
      ...
      <h2 style="color: #1B2E4B; margin: 0 0 16px;">${businessName} had ${viewCount.toLocaleString()} visitors on The Lakes Guide this week</h2>
      <p style="color: #666; margin: 0 0 16px; line-height: 1.6;">
        Hi ${userName || "there"},
      </p>
      <p style="color: #666; margin: 0 0 16px; line-height: 1.6;">
        Your The Lakes Guide listing had <strong>${viewCount.toLocaleString()}</strong> views last week. ${shareLine ?? "People are finding you."}
      </p>
      <p style="color: #666; margin: 0 0 16px; line-height: 1.6;">
...
export function getUpgradeNudgeSubject(businessName: string, viewCount: number): string {
  return `${businessName} had ${viewCount.toLocaleString()} visitors on The Lakes Guide this week`;
}
