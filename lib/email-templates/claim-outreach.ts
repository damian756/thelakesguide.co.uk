...
<head>
  ...
  <title>Your business on SouthportGuide</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #f5f5f5;">
  <div style="max-width: 560px; margin: 0 auto; padding: 24px;">
    <div style="background: white; border-radius: 12px; padding: 24px; border: 1px solid #eee;">
      ...
      <h2 style="color: #1B2E4B; margin: 0 0 16px;">Your business had ${viewCount.toLocaleString()} visitors on SouthportGuide this month</h2>
      <p style="color: #666; margin: 0 0 16px; line-height: 1.6;">
        Hi,
      </p>
      <p style="color: #666; margin: 0 0 16px; line-height: 1.6;">
        Your listing for <strong>${businessName}</strong> on SouthportGuide.co.uk had <strong>${viewCount.toLocaleString()}</strong> visitors in the last 30 days. People are looking for you. Your phone number, website and opening hours are the first things they look for.
      </p>
...
export function getClaimOutreachSubject(businessName: string, viewCount: number): string {
  return `Your business had ${viewCount.toLocaleString()} visitors on SouthportGuide this month`;
}
